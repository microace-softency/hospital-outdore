                ...
                {groupTab !== null ? productGroups[groupTab]?.items?.map((item) => (
                  <div className=' bg-sky-300 hover:bg-slate-300 cursor-pointer border-b-[1px] border-slate-400 w-full '>
                    ...
                    <div className='w-max ms-auto'>
                      <span className='text-sm font-semibold ml-2 text-slate-500 ml-12'>{item.MRP_RATE * (addedItems.find(obj => obj.DESCRIPT === item.DESCRIPT)?.QUANTITY || 0)}.00</span>
                      <span className='text-sm font-semibold ml-2 text-slate-500'>({item.MRP_RATE}.00)</span>
                    </div>
                  </div>
                )) :
        ...
        <div className='max-w-full overflow-x-scroll'>
          <div className='flex h-[550px] min-w-max rounded-xl overflow-hidden bg-sky-300 drop-shadow py-2'>
            <div className='min-w-[210px] max-w-max overflow-y-scroll'>
              ...
              {!loading &&
                <div
                  className={`cursor-pointer hover:bg-slate-300 border-b-[1px] border-slate-400  p-2 w-full bg-sky-300 ${!selectedGroup ? 'bg-slate-300' : ''}`}
            ...
            <div className='bg-sky-300 px-2 max-w-[500px]' >
                  ...
                  <table className='w-full bg-white border-collapse'>
                    ...
                    <tbody>
                      {addedItems.map((item, i) => (
                        <tr key={i}
                          onClick={() => { setSearchTerm(item.DESCRIPT); setSelectedGroup(null) }}
                          className={`hover:bg-white cursor-pointer ${i % 2 === 0 ? 'bg-slate-300' : 'bg-sky-300'}`}>
                          <td className='py-2 px-4 w-52'>{item.DESCRIPT}</td>
