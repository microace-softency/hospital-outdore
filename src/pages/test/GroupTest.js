...
import CreateGroupTest from "../../components/CreteGroupTest";
...
function GroupTest() {
  ...
  const loadData = async () => {
    ...
    try {
      const response = await axios.get(
        "http://localhost:8005/api/test/grouptest"
      );
  ...
  const deleteContact = (id) => {
    if (
      window.confirm("Are you sure that you wanted to delete that Test Group ?")
    ) {
      axios.delete(`http://localhost:8005/api/test/removegrouptest/${id}`);
      toast.success("Test Group Delete Successfully");
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: (row) => (
        <ul>
          {row.row.original.subGroups.map((subGroup, index) => (
            <li key={index}>
