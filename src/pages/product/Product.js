...
const deleteContact = (id) =>{
  if(window.confirm("Are you sure that you wanted to delete that Doctor ?")){
      axios.delete(`http://localhost:8005/api/product/removeproduct/${id}`);
...
console.log('data', data);
  ...
  return (
    <div className='container p-2 '>
      ...
      {showCreateForm ? (
        <CreateProduct onCancel={() => { setShowCreateForm(false) }} customers={data} model={model}/>
      ) : (
      <div className='flex justify-between'>
        <Button className='mb-2 drop-shadow' onClick={() => setShowCreateForm(true)}> Add Medicine</Button>
        </div>
      )}
      {isLoading && !data[0] &&
      ...
      {showCreateForm ? null :
        (data[0] ?
          ...
          (!isLoading ? <p className='text-red-400 font-semibold w-max mx-auto my-20'>No Medicine's Data</p>
            : '')
        )}
    </div>
  );
