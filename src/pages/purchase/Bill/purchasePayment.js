...
function PurchasePayment() {
  ...
  const handlePaymentReceived = async () => {
    ...
    const bill = {
      ...
      BASIC: totalProductAmount.toFixed(2) || '',
      TERMTOTAL: totalGSTAmount.toFixed(2) || '',
      DISCOUNT: discountAmount.toFixed(2) || '',
      NET_AMT: totalBillAmount.toFixed(2) || '',
    };
    const billTerm = {
      BILL_NO: data.invoiceNumber || '',
      BILL_DATE: data.paymentDate || '',
    ...
    const entries = [
      { collectionName: myCollection(collections.BILLIN), data: bill },
      { collectionName: myCollection(collections.BLINTERM), data: billTerm },
    ];
    // Create an array to store product entries
    const productEntries = state.addedProducts.map((product) => {
      return {
        collectionName: myCollection(collections.BLLINDET),
        data: {
          BILL_NO: data.invoiceNumber || '',
          BILL_DATE: date || '',
          CUSTNAME: data.customer.NAME || '',
          ...
          PRODTOTAL: (product.MRP_RATE * product.QUANTITY).toFixed(2) || '',
          DISCOUNTPER: product.DISCPER || '',
          DISCOUNTAMT: ((product.MRP_RATE * product.QUANTITY * product.DISCPER) / 100).toFixed(2) || '',
          AMOUNT: product.MRP_RATE * product.QUANTITY - ((product.MRP_RATE * product.QUANTITY * product.DISCPER) / 100).toFixed(2) || '',
          IGSTPER: product.IGST || '',
          IGSTAMT: (((product.MRP_RATE * product.QUANTITY - ((product.MRP_RATE * product.QUANTITY * product.DISCPER) / 100)) * (product.IGST)) / 100).toFixed(2) || '',
          CGSTAMT: (((product.MRP_RATE * product.QUANTITY - ((product.MRP_RATE * product.QUANTITY * product.DISCPER) / 100)) * (product.IGST / 2)) / 100).toFixed(2) || '',
          SGSTAMT: (((product.MRP_RATE * product.QUANTITY - ((product.MRP_RATE * product.QUANTITY * product.DISCPER) / 100)) * (product.IGST / 2)) / 100).toFixed(2) || '',
          GSTAMT: (((product.MRP_RATE * product.QUANTITY - ((product.MRP_RATE * product.QUANTITY * product.DISCPER) / 100)) * (product.IGST)) / 100).toFixed(2) || '',
          TOTALAMT: (((product.MRP_RATE * product.QUANTITY - ((product.MRP_RATE * product.QUANTITY * product.DISCPER) / 100)) + (((product.MRP_RATE * product.QUANTITY - ((product.MRP_RATE * product.QUANTITY * product.DISCPER) / 100)) * (product.IGST)) / 100))).toFixed(2)
        },
      };
    });
    multiEntry(entries);
  ...
  return (
    <div className="container mx-auto m-8">
      <div className="bg-white p-6 rounded-lg shadow-lg printable-content" id="printableContent">
        <div className='flex justify-start p-2 bg-sky-300 text-slate-500 -mx-6 px-4'>
          {isBillingMode ? <h1 className=''>TAX INVOICCE</h1> : <h1 className="text-3xl font-semibold mb-4 ">TAX INVOICE CONFIRMATION</h1>}
        </div>
        {/* Customer Details */}
        {customerInfo && (
          <div className=''>
            {isBillingMode &&
              <div className='flex justify-between flex-col py-2'>
                <p className='m-1'><strong>Bill No :</strong> {customerInfo?.invoiceNumber}</p>
                <p className='m-1'><strong>Date :</strong> {formatDateTime(customerInfo?.paymentDate)}</p>
              </div>
            }
              ...
              <table className="min-w-full border-collapse">
                ...
                <tfoot>
                  <tr className="bg-gray-400">
                    ...
                    {/* <td className="py-2 px-3 sm:px-2 font-semibold">₹{(totalProductAmount - discountAmount).toFixed(2)}</td>
                    <td className="py-2 px-3 sm:px-2"></td> */}
        ...
        <div className='flex justify-center mt-20'>
          {!isBillingMode ?
            (<button
              ...
              className="bg-green-700 drop-shadow hover:shadow-xl text-white px-4 py-2 rounded-md hover:bg-green-700 focus:outline-none focus:bg-green-600"
            >
              Proceed 
            </button>)
            :
            (
              <div className=''>
                ...
                <button
                  onClick={() => { window.print(); navigate('/purchase/bill')}}
