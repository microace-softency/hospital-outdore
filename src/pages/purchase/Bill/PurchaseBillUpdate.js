...
function PurchaseBillUpdate() {
  ...
  const handlePaymentReceived = async () => {
    ...
    const invoiceNumber = state.bill.BILL_NO;
    const currentDate = state.bill.BILL_DATE;
    ...
    deleteMultipleDocs(myCollection(collections.BILLIN), 'BILL_NO', state.bill.BILL_NO)
    deleteMultipleDocs(myCollection(collections.BLLINDET), 'BILL_NO', state.bill.BILL_NO)
    deleteMultipleDocs(myCollection(collections.BLINTERM), 'BILL_NO', state.bill.BILL_NO)
      .then((res) => {
        multiEntry(entries);
