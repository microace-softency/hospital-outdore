...
function PurchaseOrderUpdate() {
  ...
  const handlePaymentReceived = async () => {
    ...
    const entries = [
      { collectionName: myCollection(collections.PORDER), data: bill },
    ...
    const productEntries = state.addedProducts.map((product) => {
      return {
        collectionName: myCollection(collections.PORDERDET),
        data: {
          BILL_NO: data.invoiceNumber  || '',
          BILL_DATE: date  || '',
          CUSTNAME: data.customer.NAME  || '',
    ...
    deleteMultipleDocs(myCollection(collections.PORDER), 'BILL_NO', state.bill.BILL_NO)
    deleteMultipleDocs(myCollection(collections.PORDERDET), 'BILL_NO', state.bill.BILL_NO)
    deleteMultipleDocs(myCollection(collections.PORDERTERM), 'BILL_NO', state.bill.BILL_NO)
      .then((res) => {
        multiEntry(entries);
