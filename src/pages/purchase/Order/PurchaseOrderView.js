...
function PurchaseOrderView() {
  ...
  useEffect(() => {
    ...
    setIsLoading(true)
    fetchDataWithWhere(myCollection(collections.PORDER), matchField, billNo)
    ...
    fetchDataWithWhere(myCollection(collections.PORDERDET), matchField, billNo)
      .then((servicefetch) => {
  ...
  return (
    <div className="container p-2">
      <Row className='flex justify-between w-full'>
        <Col sm={12} md={12} lg={6}><h2 className="text-xl font-bold mx-2">Purchase Order</h2></Col>
      </Row>
      {isLoading ?
        <div className='w-full h-60 flex justify-center items-center'>
          ...
          <div className='w-full mx-auto bg-white rounded-xl'>
            <ViewCustDetails />
            <ViewProdDetails modelFor="/purchase/order/update" />
          </div>
          {state.modify &&
          <div className='flex justify-end my-2'>
            <Button
