...
function PurchaseConfirm() {
  ...
  const handleConfirm = async () => {
    ...
    const entries = [
      ...
      { collectionName: myCollection(collections.PORDERTERM), data: billTerm },
    ];
        ...
        <div className="bg-teal-800 p-2 rounded-md m-0">
          ...
          <Modal.Body className="flex justify-between items-center">
            ...
            <div className="flex justify-between w-1/2 m-0">
              <Button variant="danger" onClick={()=> {navigate('/purchase/order')}} className="px-4">
                No
              </Button>
              <Button variant="success" onClick={() => {navigate('/purchase/order');window.print()}} className="px-4">
                Yes
              </Button>
            </div>
          </Modal.Body>
