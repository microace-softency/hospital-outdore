...
function PurchaseOrder() {
  ...
  const collectionName = myCollection(collections.PORDER)
  ...
  const handleDelete = (id) => {
    deleteMultipleDocs(myCollection(collections.PORDER), 'BILL_NO', id)
    deleteMultipleDocs(myCollection(collections.PORDERDET), 'BILL_NO', id)
    deleteMultipleDocs(myCollection(collections.PORDERTERM), 'BILL_NO', id)
    .then(() => {
        getData();
  ...
  return (
    <div className='container p-2 '>
      ...
      <Modal className="" show={showDeleteModal} onHide={cancelDelete}>
        ...
        <Button className='mb-2 drop-shadow' onClick={() => navigate('/purchase/order/add')}> Add </Button>
      {isLoading && !data[0] &&
